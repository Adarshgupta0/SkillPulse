    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await axios.post(
                `${import.meta.env.VITE_BACKEND_URL}/user/otp-verify`,
                userData,
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 space-y-6">
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                            <div className="space-y-4">
                                {/* Email Input */}
                                <div>
                                    <label htmlFor="userotp" className="block text-sm font-medium text-gray-700 mb-2">
                                        Enter OTP
                                    </label>
                                    <input
                                        type="number"
                                        ...
                                        {...register("userotp", { required: { value: true, message: "OTP is required" }, minLength: { value: 6, message: "min length 6" } })}
                                    />
                                    <div className='flex items-center justify-end'>
                                        {errors.userotp && <span className='text-red-600  text-sm '>{errors.userotp.message}</span>}
                                    </div>
                                </div>
