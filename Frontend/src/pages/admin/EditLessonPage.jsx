...
const EditLessonPage = () => {
    ...
    useEffect(() => {
        const found = adminaesson.find((r) => r._id === id);
        if (found) {
            setlesson(found);
            reset(found);
        }
    }, [id, adminaesson, reset]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await axios.put(
                `${import.meta.env.VITE_BACKEND_URL}/admin/lesson-update/${id}`,
                formData,
                { withCredentials: true }
            );

            if (response.data.success) {
                setAdminLesson(response.data.lessons);
                reset();
                                        ...
                                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    id="roadmapType"
                                                    required
                                                    defaultValue={lesson?.lesson_category}
                                                    className="w-full px-3 py-2 border border-gray-300 bg-white text-gray-900 rounded-sm outline-none"
