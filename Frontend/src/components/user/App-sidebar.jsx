...
import { Home, User, Brain, ChevronRight, X, Menu, LogOut, Youtube, BookOpenCheck, MapPinned, } from "lucide-react"
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-300 flex-shrink-0">
          <Link to='/' className="flex items-center h-[31px] gap-2">
        ...
        <div className="border-t  border-gray-200 p-4 flex-shrink-0">
          <div className="flex items-center gap-2">
            <Link to='/profile' className="h-8 w-8  rounded-full bg-gray-300 flex items-center justify-center">
              <User className="h-4 w-4 text-gray-600" />
            </Link>
            <Link to='/profile' className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">{user.name}</span>
              <span className="text-xs text-gray-500">{user.userCategory}</span>
            </Link>
          </div>
        </div>
      </div>
